...
for _, item in ipairs(BabyfurnitureItems) do table.insert(allItems, {name = item, isBaby = true}) end
...
if furnitureFolder then
    ...
    for _, item in ipairs(allItems) do
        ...
        if foundItem then
            ...
            -- Special case for baby furniture
            if item.isBaby then
                local useBlock = findDeep(foundItem, "UseBlock")
                if useBlock and useBlock:IsA("BasePart") then
                    cframe = useBlock.CFrame
                    print("Using CFrame from UseBlock for " .. itemName)
                end
            end
